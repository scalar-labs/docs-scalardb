...
Another method for executing administrative operations is to use [Schema Loader](schema-loader/README.mdx).
...
By adding the following configuration, `Get` and `Scan` operations results will contain [transaction metadata](schema-loader/README.mdx#internal-metadata-for-consensus-commit):
