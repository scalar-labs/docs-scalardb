...
<Tabs groupId="databases" queryString>
  ...
  <TabItem value="Cosmos DB for NoSQL" label="Cosmos DB for NoSQL">
...
You need to define the database schema (the method in which the data will be organized) in the application. For details about the supported data types, see [Data type mapping between ScalarDB and other databases](schema-loader/README.mdx#data-type-mapping-between-scalardb-and-the-other-databases).
...
The `--coordinator` option is specified because a table with `transaction` set to `true` exists in the schema. For details about configuring and loading a schema, see [ScalarDB Schema Loader](schema-loader/README.mdx).
