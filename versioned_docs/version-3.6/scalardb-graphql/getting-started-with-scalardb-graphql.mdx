...
In this Getting Started guide, you will be running a GraphQL server on your local machine.

## Prerequisites

Here we assume that you have already installed Docker and have access to a ScalarDB-supported database such as Cassandra. Please configure them first by following [Getting Started with ScalarDB](../getting-started.mdx) if you have not set them up yet.
...
The following command starts up the ScalarDB GraphQL server. At the first time you run the command, it will download the Docker image from GitHub Container Registry.
...
ScalarDB GraphQL always run queries with transactions. The above query starts a new transaction, executes a ScalarDB Put command, and commits the transaction at the end of the execution.
...
The generated schema provides the `@trasaction` directive that allows you to identify transactions. This directive can be used with both queries and mutations.
...
To run the next queries or mutations in the transaction you started, specify the transaction ID as the `id` argument of the `@transaction`. The following example will update two accounts that you got in the previous example in the same transaction. This represents a transfer of balance from user1's account to user2's account.
