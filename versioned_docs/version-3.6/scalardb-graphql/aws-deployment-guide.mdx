...
Please follow steps **1** to **3** in the [Deploy ScalarDL on AWS](../scalar-kubernetes/ManualDeploymentGuideScalarDLOnAWS.mdx) guide to set up a VPC, subnets, a bastion host, a database, an EKS cluster, and a node group.
