...
One of the keys to achieving storage-agnostic or database-agnostic ACID transactions on top of existing storage and database systems is the storage abstraction capabilities that ScalarDB provides. Storage abstraction defines a [data model](./data-modeling.mdx) and the APIs (Storage API) that issue operations on the basis of the data model.

Although you will likely use the [Transaction API](api-guide.mdx#transaction-api) in most cases, another option is to use the Storage API.
...
For details about creation options, see [Creation options](./api-guide.mdx#creation-options-1).
...
For details about creation options, see [Creation options](./api-guide.mdx#creation-options-2).
