...
For detailed deployment instructions, see [Deploy ScalarDB Analytics Server](./deploy-scalardb-analytics-server.mdx).
...
<h3>Run interactive queries by using notebooks</h3>
...
<h4>Run interactive queries by using notebooks</h4>
...
- `<YOUR_LICENSE_CERT_PEM>`: Your license verification certificate. For guidance on how to set up your license, including both commercial and trial licenses, see [How to Configure a License Key](../scalar-licensing/index.mdx).
