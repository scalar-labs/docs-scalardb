...
This release improves Cosmos DB error handling, adds retry handling for conflict errors during schema operations, improves thread pool management for two-phase commit transactions in ScalarDB SQL, and upgrades the AWS SDK and grpc_health_probe to address security vulnerabilities.
