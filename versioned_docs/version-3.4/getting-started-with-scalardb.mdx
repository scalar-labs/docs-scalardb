...
[`ElectronicMoneyWithStorage.java`](https://github.com/scalar-labs/scalardb/blob/3.4/docs/getting-started/src/main/java/sample/ElectronicMoneyWithStorage.java)
...
To use transaction, we can just add a key `transaction` and value as `true` in the ScalarDB schema we used.
...
You don't need to set a key `transaction` to `true` in ScalarDB schema for the native transaction manager.
...
* [Requirements in the Underlying Databases](requirements.mdx)
* [Database Schema in ScalarDB](schema.mdx)
* [Schema Loader](schema-loader/README.mdx)
