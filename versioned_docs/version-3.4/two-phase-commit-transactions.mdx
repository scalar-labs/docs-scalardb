...
You don't need a special configuration for Two-phase Commit Transactions, so you can follow [the ScalarDB Server document](scalardb-server.mdx) to use it.
...
- [Microservice Transaction Sample](scalardb-samples/microservice-transaction-sample/README.mdx)
