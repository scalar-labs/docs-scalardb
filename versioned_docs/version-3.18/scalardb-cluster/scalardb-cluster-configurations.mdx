...
- **Description:** The maximum number of authentication tokens that the client caches. The client caches a token for each username and password pair so that it does not need to authenticate on every request. Each cached token expires one day after it is cached. The cache is shared between the credentials set in the configuration properties and the credentials passed programmatically. This configuration applies only when the authentication type is `userpass`.
- **Default value:** `10000`
