...
If `--coordinator` is specified, a [Coordinator table](api-guide.mdx#specify-operations-for-the-coordinator-table) will be created. When [group commit](consensus-commit.mdx#group-commit) is enabled, this command creates a Coordinator table with the additional column that group commit requires.
...
If you enable [group commit](consensus-commit.mdx#group-commit) after creating the Coordinator table, running this command with `--coordinator` also adds the additional column that group commit requires.
