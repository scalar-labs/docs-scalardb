...
export default function GoogleAIModeSearch() {
  ...
  const getPlaceholderText = useCallback(() => {
    return currentLanguage === 'ja-jp'
      ? 'ScalarDB について聞いてみる...'
      : 'Ask a question about ScalarDB...';
  }, [currentLanguage]);

  // Localized modal text
  const modalTitle = currentLanguage === 'ja-jp' ? 'AI に質問する' : 'Ask AI';
  const legalNotice = currentLanguage === 'ja-jp'
    ? 'この AI ボットの体験は実験的なものです。AI によって生成された回答は必ずしも正確ではない場合があるため、必ず公式の ScalarDB ドキュメントもご参照ください。また、回答は最新バージョンのドキュメントに基づいています。機密情報 (API キーやパスワードなど) は絶対に入力しないでください。'
    : 'This AI bot experience is experimental. Responses are generated by AI and may not be fully accurate, so always consult the official ScalarDB documentation. In addition, responses are based on the latest version of the documentation. Please do not share confidential information (like API keys or passwords).';
  const googleDomain = currentLanguage === 'ja-jp' ? 'https://www.google.co.jp' : 'https://www.google.com';
