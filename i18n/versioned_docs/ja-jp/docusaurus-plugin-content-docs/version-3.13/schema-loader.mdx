...
<Tabs groupId="databases" queryString>
  ...
  <TabItem value="Cosmos_DB_for_NoSQL" label="Cosmos DB for NoSQL">
    ...
    - `<COSMOS_DB_FOR_NOSQL_KEY>` では、プライマリーキーまたはセカンダリキーを使用できます。
    - `-r BASE_RESOURCE_UNIT` はオプションです。各データベースの RU を指定できます。データベース内のテーブルの最大 RU が設定されます。テーブルの RU を指定しない場合は、このオプションでデータベース RU が設定されます。デフォルトでは 400 です。
  </TabItem>
  <TabItem value="Cassandra" label="Cassandra">
    ```console
