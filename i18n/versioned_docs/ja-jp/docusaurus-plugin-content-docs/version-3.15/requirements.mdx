...
ScalarDB Core および Cluster で**管理されていない**データベースについては、データソースに対する読み取り権限を持つユーザーでデータソースを登録してください。データソースの登録手順については、[データソースの設定](scalardb-analytics/run-analytical-queries#データソースの設定)を参照してください。
