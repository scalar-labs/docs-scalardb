...
`transaction` が `true` に設定されたテーブルがスキーマに存在するため、`--coordinator` オプションが指定されています。スキーマの設定と読み込みの詳細については、[ScalarDB  Schema Loader](schema-loader.mdx) を参照してください。
...
次のコマンドを実行して、顧客 ID `1` の `credit_total` の残額を使用してマンゴー1個を注文します。
...
顧客 ID `1` が支払いを済ませたので、次のコマンドを実行してブドウ1個とマンゴー1個を注文します。
