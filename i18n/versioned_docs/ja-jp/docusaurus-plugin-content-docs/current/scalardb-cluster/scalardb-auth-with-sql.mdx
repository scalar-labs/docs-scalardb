...
- **OIDC (`OIDC`):** クライアントアプリケーションは、パスワードの代わりに OIDC プロバイダー (例: Keycloak) からの JWT アクセストークンを渡します。ABAC と組み合わせることで、JWT クレームがリクエストごとに動的にアクセスを制限できます。詳細については、[OIDC ベースの JWT アクセストークンを用いてユーザアクセスを制御する](./control-access-via-oidc-based-jwt-tokens.mdx)を参照してください。
