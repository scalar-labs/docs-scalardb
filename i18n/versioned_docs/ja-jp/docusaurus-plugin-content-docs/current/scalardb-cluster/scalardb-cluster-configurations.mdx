...
ScalarDB Cluster の OIDC 統合に使用できる設定は以下の通りです。OIDC 統合の詳細については、[OIDC ベースの JWT アクセストークンを用いてユーザアクセスを制御する](./control-access-via-oidc-based-jwt-tokens.mdx)を参照してください。
...
- **説明:** 認証タイプ。`userpass` はユーザー名とパスワードでの認証を使用します。`oidc_jwt` は OIDC JWT ベースのアクセス制御を使用します。`oidc_jwt` の詳細については、[OIDC ベースの JWT アクセストークンを用いてユーザアクセスを制御する](./control-access-via-oidc-based-jwt-tokens.mdx)を参照してください。
