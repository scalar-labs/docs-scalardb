...
| `scalar.db.cluster.client.scan_fetch_size`                         | `Scanner` がクラスタからデータを取得するために使用するフェッチサイズ。これは、`Scanner` がクラスタから一度に取得するレコードの数です。大きな値を指定すると、クラスタへのアクセスの往復回数が減るためパフォーマンスが向上する可能性がありますが、メモリ使用量が増加する可能性もあります。               | `10`            |
...
| `scalar.db.transaction.enabled`                                    | トランザクション機能が有効かどうか。例えば、Embedding 機能のみを使用する場合、このプロパティを `false` に設定できます。                                                                                                                                            | `true`           |
| `scalar.db.cluster.node.scanner_management.expiration_time_millis` | ScalarDB Cluster ノードは進行中のスキャナーを管理しており、長時間アイドル状態のスキャナーを期限切れにしてリソースリークを防ぎます。この設定では、このスキャナー管理機能の有効期限（ミリ秒単位）を指定します。                                                                     | `60000` (60秒)   |
...
| `scalar.db.consensus_commit.one_phase_commit.enabled`                                  | 1フェーズコミット最適化が有効かどうか。                                                                                                                                                                                                                              | `false`                                                           |
| `scalar.db.consensus_commit.coordinator.write_omission_on_read_only.enabled`           | 読み取り専用トランザクションに対して Coordinator テーブルへの書き込み省略最適化を有効にするかどうか。この最適化は、データを変更しない読み取り専用トランザクションに有効であり、Coordinator テーブルへの不要な書き込みを回避します。                                                      | `true`                                                            |
...
| 名前                         | 説明                                                                                                                                                                                                                             | デフォルト |
|-----------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |-----------|
| `scalar.db.scan_fetch_size` | ストレージのスキャン操作時に一度に取得するレコード数を指定します。値を大きくすると、大きな結果セットの場合にストレージへのアクセスの往復回数が減りパフォーマンスが向上しますが、メモリ使用量も増加します。値を小さくするとメモリ使用量は減りますが、レイテンシが増加する場合があります。 | `10`     |
