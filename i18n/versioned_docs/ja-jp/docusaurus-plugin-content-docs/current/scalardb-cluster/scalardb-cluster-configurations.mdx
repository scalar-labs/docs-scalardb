...
以下は、プリミティブインターフェースを使用する ScalarDB Cluster クライアントの認証と認可の設定です。

Java client SDK とプリミティブインターフェースを使用するアプリケーションやツール([クラスター用 Schema Loader](./developer-guide-for-scalardb-cluster-with-java-api.mdx#クラスター用-schema-loader) や [ScalarDB ベンチマークツール](../scalardb-benchmarks/README.mdx)など) を実行する場合、特にクラスターで認証と認可が有効になっている場合は、クライアント側の `database.properties` ファイルに次の設定を行う必要があります。
...
- **フィールド:** `scalar.db.cluster.tls.enabled`
- **説明:** クライアントでワイヤ暗号化 (TLS) が有効になっているかどうか。
- **デフォルト値:** `false`
...
ScalarDB Cluster クライアントの認証と認可の設定は以下の通りです。
