...
## JWT アクセストークンをプログラムで渡す

設定プロパティに JWT アクセストークンを設定する代わりに、以下の方法でプログラムで渡すことができます:

- **スレッドごと:** `OidcJwtAccessTokenHolder` を使用して、コードブロックの実行中に現在のスレッドのトークンを設定します。
- **トランザクション、操作、または SQL ステートメントごと:** auth 属性 `auth-type` と `auth-oidc-jwt-access-token` を使用します。

どちらのメカニズムも `userpass` および `oidc_jwt` 認証タイプで同じように機能し、認証と認可ガイドの[クレデンシャルをプログラムで渡す](./scalardb-auth-with-sql.mdx#クレデンシャルをプログラムで渡す)で説明されています。`OidcJwtAccessTokenHolder` を使用した完全なアプリケーション例については、以下のチュートリアルの [7. Java クライアントでの接続](#7-java-クライアントでの接続)を参照してください。
