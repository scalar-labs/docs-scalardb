...
- Java API を使用した [名前空間の修復](#名前空間を修復する)と[テーブルの修復](#テーブルを修復する)
- ScalarDB Schema Loader を使用した[名前空間とテーブルの修復](../schema-loader.mdx#名前空間とテーブルを修復する)
...
<Tabs groupId="databases" queryString>
  ...
  <TabItem value="Object_Storage" label="Object Storage">
    使用可能なオプションはありません。
  </TabItem>
</Tabs>
...
ScalarDB のデータモデルの詳細については、[データをモデル化する](../data-modeling.mdx)を参照してください。
...
<Tabs groupId="databases" queryString>
  <TabItem value="JDBC_databases" label="JDBC データベース" default>
    | 名前       | 説明                             | デフォルト |
    |------------|----------------------------------|------------|
    | transaction-metadata-decoupling | Consensus Commitを使用している場合に、[トランザクションメタデータ分離](../consensus-commit.mdx#トランザクションメタデータ分離)を有効にします。これにより、トランザクションメタデータがアプリケーションデータとは別のテーブルで管理されます。 | false   |
  </TabItem>
  <TabItem value="DynamoDB" label="DynamoDB">
    | 名前       | 説明                             | デフォルト |
    |------------|----------------------------------|------------|
...
Consensus Commit を使用している場合、非プライマリキー列で `createIndex()` を使用すると、コンパニオン before-image セカンダリインデックスも作成されます。詳細については、[インデックスベース読み取りの正確性](../consensus-commit.mdx#インデックスベース読み取りの正確性)を参照してください。
...
Consensus Commit を使用している場合、`dropIndex()` はコンパニオン before-image セカンダリインデックスも削除します。詳細については、[インデックスベース読み取りの正確性](../consensus-commit.mdx#インデックスベース読み取りの正確性)を参照してください。
...
Consensus Commit を使用している場合、以前のバージョンから ScalarDB 3.16.5、3.17.3、または 3.18.0 にアップグレードした後、インデックスベース読み取りに必要なコンパニオン before-image セカンダリインデックスを作成するために、既存の各テーブルで `repairTable()` を実行する必要があります。詳細については、[インデックスベース読み取りの正確性](../consensus-commit.mdx#インデックスベース読み取りの正確性)を参照してください。
...
Coordinator テーブルは、[Transaction API](#transaction-api) によってトランザクションのステータスを追跡するために使用されます。
...
Consensus Commitを使用している場合に、運用環境で ScalarDB にテーブルをインポートする場合は、データベーステーブルと ScalarDB メタデータテーブルにトランザクションメタデータ列が追加されるため、慎重に計画する必要があります。この場合、データベースと ScalarDB の間にはいくつかの違いがあり、いくつかの制限もあります。詳細については、[ScalarDB Schema Loader を使用して既存のテーブルを ScalarDB にインポートする](../schema-loader-import.mdx)を参照してください。

また、`transaction-metadata-decoupling` オプションを `true` に指定することで、*トランザクションメタデータ分離*を有効にして、トランザクションメタデータをアプリケーションデータとは別のテーブルに保存することもできます。詳細については、[トランザクションメタデータ分離](../schema-loader.mdx#トランザクションメタデータ分離)を参照してください。

:::

### 認証と認可 API

ScalarDB Cluster は認証と認可をサポートしています。ユーザー、ロール、権限を Java を通じてプログラムで管理するには、<JavadocLink packageName="scalardb-cluster-java-client-sdk" path="com/scalar/db/cluster/client" className="ClusterClientTransactionAdmin" /> を使用します。これは <JavadocLink packageName="scalardb" path="com/scalar/db/api" className="AuthAdmin" /> を実装しています。

ユーザー、ロール、権限などの認証と認可の概念については、[ユーザーの認証と認可](./scalardb-auth-with-sql.mdx)を参照してください。
...
また、`AuthenticationMethod` 列挙型を使用して、ユーザーに対して1つ以上の認証方法を指定できます:

| 認証方法   | 説明                               |
|------------|------------------------------------|
| `USERPASS` | ユーザー名とパスワードによる認証。 |
| `OIDC`     | OpenID Connect (OIDC) 認証。       |
...
これらの権限名は、[ScalarDB SQL の DCL](../scalardb-sql/grammar.mdx#dcl) で使用される SQL レベルの権限名 (`SELECT`、`INSERT`、`UPDATE` など) とは異なります。Java API を直接使用する場合は、上記の `Privilege` 列挙型の値を使用してください。
...
たとえば、名前空間レベルで付与された `READ` 権限を特定のテーブルに対して取り消しても、効果はありません。[ユーザーが権限を持っているかどうかを確認する](#ユーザーが権限を持っているかどうかを確認する) で説明されているように、テーブルレベルの権限チェックでは名前空間レベルの権限も考慮されるため、テーブルスコープでの取り消しが行われた後も、名前空間全体への付与によってそのテーブルへのアクセスは引き続き許可されます。
...
ロールは次のようにユーザーから取り消すことができます。

```java
admin.revokeRoleFromUser("username", "rolename");
```

##### ロールに対するユーザーの admin オプションを取り消す

ロール自体は取り消さずに、ユーザーの admin オプションのみを次のように取り消すことができます。
...
特定のロールが付与されているユーザーは次のように取得できます。
...
ロールにメンバーロールを付与すると、そのロールを持つユーザーはメンバーロールの権限を継承します。次のように付与できます。
...
メンバーロールは次のようにロールから取り消すことができます。

```java
admin.revokeRoleFromRole("rolename", "memberrolename");
```

##### 別のロールに対するロールの admin オプションを取り消す

ロール間の付与について、admin オプションのみを次のように取り消すことができます。
...
名前空間内のすべてのテーブル、または特定のテーブルに対して、ロールに次のように権限を付与できます。
...
名前空間内のすべてのテーブル、または特定のテーブルについて、ロールから次のように権限を取り消すことができます。
...
名前空間内のすべてのテーブル、または特定のテーブルについて、ロールが持つ権限を次のように取得できます。
...
`getRolePrivileges()` は、ロールに直接付与された権限のみを返します。メンバーロールから継承された権限や、テーブルに適用される名前空間レベルの権限は含まれません。

:::

これらの API からの失敗の処理の詳細については、[認証と認可の例外を処理する](#認証と認可の例外を処理する)を参照してください。

## Transaction API

このセクションでは、ScalarDB の Transaction API を使用してトランザクション操作を実行する方法について説明します。
...
##### 属性を指定してトランザクションを開始またはスタートする

トランザクションを開始またはスタートするときに、トランザクションスコープの属性のマップを指定できます。指定した属性はトランザクション内で発行されるすべてのオペレーションにマージされるため、トランザクション全体に一律に適用する設定を開始時に一度だけ指定できます。オペレーションにすでに同じ属性キーが設定されている場合は、オペレーション側の属性が優先されます。

次のように属性を指定してトランザクションを開始またはスタートできます。
...
次のように、トランザクション ID と属性を一緒に指定することもできます。
...
`beginReadOnly` メソッドと `startReadOnly` メソッドにも、属性を受け取るオーバーロードがあります。
...
利用可能な属性の一覧については、[オペレーション属性](#オペレーション属性)を参照してください。
...
まず `Get` オブジェクトを作成し、次に以下のように `transaction.get()` メソッドを使用してオブジェクトを実行する必要があります。
...
`where()` メソッドを使用して任意の条件を指定することもできます。取得したレコードが `where()` メソッドで指定された条件と一致しない場合は、`Optional.empty()` が返されます。`where()` メソッドの引数として、条件、AND 条件セット、または OR 条件セットを指定できます。`where()` メソッドを呼び出した後、次のように `and()` メソッドまたは `or()` メソッドを使用して、さらに条件または条件セットを追加できます。
...
`where()` メソッドを使用して任意の条件を指定することもできます。詳細については、[`WHERE` 句を使用する](#where-句を使用する-1)を参照してください。
...
次のように、`all()` メソッドを呼び出した後に `where()` メソッドと `orderings()` メソッドを呼び出して、任意の条件と順序を指定できます。
...
`WHERE` 句の詳細については、[`WHERE` 句を使用する](#where-句を使用する-1)を参照してください。

###### 操作単位でクロスパーティションスキャンを有効にする

上記の設定でクロスパーティションスキャンを全体的に有効にする代わりに、オペレーションにクロスパーティションスキャン属性を設定することで、特定のオペレーションに対して有効にできます。オペレーションに設定した場合、そのオペレーションについては対応する設定値よりも属性が優先されます。

次の例では、特定のオペレーションに対してクロスパーティションスキャン、フィルタリング、および順序付けを有効にしています。
...
これらの属性の詳細については、[クロスパーティションスキャン属性](#クロスパーティションスキャン属性)を参照してください。
...
まず `Put` オブジェクトを作成し、次に以下のように `transaction.put()` メソッドを使用してオブジェクトを実行する必要があります。
...
まず `Insert` オブジェクトを作成し、次に以下のように `transaction.insert()` メソッドを使用してオブジェクトを実行する必要があります。
...
まず `Upsert` オブジェクトを作成し、次に以下のように `transaction.upsert()` メソッドを使用してオブジェクトを実行する必要があります。
...
まず `Update` オブジェクトを作成し、次に以下のように `transaction.update()` メソッドを使用してオブジェクトを実行する必要があります。
...
まず `Delete` オブジェクトを作成し、次に以下のように `transaction.delete()` メソッドを使用してオブジェクトを実行する必要があります。
...
Mutate は、`Put`、`Insert`、`Upsert`、`Update`、`Delete` の複数のミューテーションを実行する操作です。

まずミューテーションオブジェクトを作成し、次に以下のように `transaction.mutate()` メソッドを使用してミューテーションを実行する必要があります。
...
Batch は、`Get`、`Scan`、`Put`、`Insert`、`Upsert`、`Update`、`Delete` の複数の操作を実行する操作です。

まず操作オブジェクトを作成し、次に以下のように `transaction.batch()` メソッドを使用して操作を実行する必要があります。
...
オペレーション属性は、オペレーションに関する追加情報を保持するためのキーと値のペアです。オペレーション属性は、オペレーションビルダーの `attribute()` メソッドや `attributes()` メソッドを使用して個々のオペレーションにオペレーションスコープの属性として設定するか、トランザクションの開始時にトランザクションスコープの属性として設定できます。トランザクションスコープの属性は、そのトランザクション内のすべてのオペレーションにマージされます。同じ属性が両方のレベルで設定されている場合は、オペレーションスコープの値がトランザクションスコープの値よりも優先されます。トランザクションスコープの属性の詳細については、[属性を指定してトランザクションを開始またはスタートする](#属性を指定してトランザクションを開始またはスタートする)を参照してください。

次の例は、個々のオペレーションにオペレーション属性を設定する方法を示しています。
...
次の属性が利用可能です。属性をサポートしていないオペレーションに属性を設定しても、効果はありません。

###### クロスパーティションスキャン属性

次の属性は、クロスパーティション `Scan` 操作 (パーティションキーを指定しない `Scan` 操作) に適用されます。オペレーションに設定した場合、そのオペレーションについては対応する `scalar.db.cross_partition_scan.*` の設定値よりも優先されます。設定の詳細については、[クロスパーティションスキャン設定](../configurations.mdx#クロスパーティションスキャン設定)を参照してください。

- **`db-cross-partition-scan-enabled`:** オペレーションでクロスパーティションスキャンを有効にするかどうか。値は `true` または `false` である必要があります。
- **`db-cross-partition-scan-filtering-enabled`:** オペレーションでフィルタリングを伴うクロスパーティションスキャンを有効にするかどうか。値は `true` または `false` である必要があります。
- **`db-cross-partition-scan-ordering-enabled`:** オペレーションで順序付けを伴うクロスパーティションスキャンを有効にするかどうか。値は `true` または `false` である必要があります。この属性は JDBC データベースでのみ利用できます。

これらの属性を設定するには、<JavadocLink packageName="scalardb" path="com/scalar/db/api" className="DatabaseOperationAttributes" /> ユーティリティクラスを使用できます。

###### Consensus Commit 属性

次の属性は、[Consensus Commit](../consensus-commit.mdx) で実行されるトランザクションに適用されます。

- **`cc-transaction-isolation`:** トランザクションの分離レベル。値は `SNAPSHOT`、`SERIALIZABLE`、または `READ_COMMITTED` である必要があります。設定すると、そのトランザクションについて設定済みのデフォルトの分離レベルを上書きします。分離レベルの詳細については、[分離レベル](../consensus-commit.mdx#分離レベル)を参照してください。

:::warning

他のオペレーション属性とは異なり、`cc-transaction-isolation` はトランザクション内のオペレーション単位で適用することはできません。この属性はトランザクションの開始時にのみ解釈されます。この属性を使用するには、[属性を指定してトランザクションを開始またはスタートする](#属性を指定してトランザクションを開始またはスタートする)ときに指定するか、[トランザクションを開始せずにトランザクションを実行する](#トランザクションを開始せずにトランザクションを実行する)場合はオペレーションに設定してください。後者の場合、ScalarDB は暗黙的に開始するトランザクションにその値を使用します。

:::

この属性を設定するには、<JavadocLink packageName="scalardb" path="com/scalar/db/transaction/consensuscommit" className="ConsensusCommitOperationAttributes" /> ユーティリティクラスを使用できます。
...
最初に `Get` オブジェクトを作成し、次に以下のように `transactionManager.get()` メソッドを使用してオブジェクトを実行する必要があります。
...
まず `Put` オブジェクトを作成し、次に以下のように `transactionManager.put()` メソッドを使用してオブジェクトを実行する必要があります。
...
まず `Insert` オブジェクトを作成し、次に以下のように `transactionManager.insert()` メソッドを使用してオブジェクトを実行する必要があります。
...
まず `Upsert` オブジェクトを作成し、次に以下のように `transactionManager.upsert()` メソッドを使用してオブジェクトを実行する必要があります。
...
`Upsert` 操作の詳細については、[`Upsert` 操作](#upsert-操作)を参照してください。

#### `Update` 操作の実行

`Update` は、トランザクションを通じて基礎となるストレージ内のエントリを更新する操作です。エントリが存在しない場合、操作によって変更は行われません。

まず `Update` オブジェクトを作成し、次に以下のように `transactionManager.update()` メソッドを使用してオブジェクトを実行する必要があります。
...
まず `Delete` オブジェクトを作成し、次に以下のように `transactionManager.delete()` メソッドを使用してオブジェクトを実行する必要があります。
...
まずミューテーションオブジェクトを作成し、次に以下のように `transactionManager.mutate()` メソッドを使用してミューテーションを実行する必要があります。
...
まず操作オブジェクトを作成し、次に以下のように `transactionManager.batch()` メソッドを使用して操作を実行する必要があります。
...
Batch 操作の詳細については、[Batch 操作](#batch-操作)を参照してください。
...
認証と認可が有効になっている場合、現在のユーザーが認証されていないか、必要な権限を持っていないために操作が失敗することがあります。Administrative API と Transaction API はどちらも、次の4つの共通のメソッドを通じてこのような失敗を報告します。

- `isAuthenticationError()` は、現在のユーザーが認証されていない場合に `true` を返します。
- `isAuthorizationError()` は、現在のユーザーが必要な権限を持っていない場合に `true` を返します。
- `isSuperuserRequired()` は、操作にスーパーユーザー権限が必要な場合に `true` を返します。
- `getRequiredPrivilege()` は、権限不足が原因で失敗した場合に、その操作に必要な権限を返します。

Administrative API では、これらのメソッドは `ExecutionException` にあります。Transaction API では、`TransactionException` およびそのサブクラスにあります。たとえば、現在のユーザーがテーブルに対する `READ` 権限を持っていない場合、`get()` は `CrudException` をスローし、この例外に対して `isAuthorizationError()` は `true` を返します。

認証と認可のステータスコードの詳細については、[認証および認可エラーコード](./scalardb-auth-status-codes.mdx)を参照してください。
