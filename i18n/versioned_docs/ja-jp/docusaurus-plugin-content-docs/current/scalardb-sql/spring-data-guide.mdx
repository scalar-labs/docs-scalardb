...
Spring Data JDBC for ScalarDB は、Spring アプリケーションの一部として使用されることになっています。次のプロパティを設定してください。

### spring.datasource.driver-class-name

これは、固定値 `com.scalar.db.sql.jdbc.SqlJdbcDriver` に設定する必要があります。常に必要です。
...
この値は、ScalarDB JDBC 接続 URL 設定に従います。詳細については、[ScalarDB JDBC ガイド](jdbc-guide.mdx)および [ScalarDB Cluster SQL クライアント設定](../scalardb-cluster/developer-guide-for-scalardb-cluster-with-java-api.mdx#scalardb-cluster-sql-クライアント設定)を参照してください。常に必要です。
...
### scalar.db.sql.default_namespace_name.existence_check.enabled (`spring.datasource.url` 内)

この設定は推奨されます。また、`scalar.db.sql.default_namespace_name` を設定する場合は必要です。その場合、`true` (デフォルト) のままにすると Spring Data JDBC の起動に失敗するため、`false` に設定してください。
...
アノテーションは次のパラメーターをサポートしています:

| パラメーター | デフォルト | 説明 |
|---|---|---|
| `basePackages` (または `value`) | アノテーション付きクラスのパッケージ | リポジトリインターフェースをスキャンするベースパッケージ。複数の `@EnableScalarDbRepositories` アノテーションを使用してスキャン競合を避ける場合に必要です。 |
| `transactionManagerRef` | `scalarDbTransactionManager` | トランザクションマネージャーの bean 名。[2フェーズコミットトランザクション](#2フェーズコミットトランザクション)の場合は `scalarDbSuspendableTransactionManager` に設定します。 |
| `jdbcOperationsRef` | `scalarDbNamedParameterJdbcTemplate` | 使用する `NamedParameterJdbcOperations` の bean 名。 |
| `dataAccessStrategyRef` | `""` (自動検出) | `DataAccessStrategy` の bean 名。複数データソース設定で必要です。 |

最もシンプルなケース (単一の ScalarDB データソース) では、デフォルトが次のセクションで説明する自動構成された bean を指すため、これらのパラメーターを設定する必要はありません。

## 自動構成された bean

Spring Data JDBC for ScalarDB は、`ScalarDbJdbcConfiguration` を通じて次の bean を自動的に登録します:

| Bean 名 | 型 | 説明 |
|---|---|---|
| `jdbcTemplate` | `JdbcTemplate` | デフォルト JDBC テンプレート (`@Primary`)。同じ型の ScalarDB 固有の bean との bean 競合を防ぐために定義されています。 |
| `namedParameterJdbcTemplate` | `NamedParameterJdbcTemplate` | デフォルト名前付きパラメーター JDBC テンプレート (`@Primary`)。同じ型の ScalarDB 固有の bean との bean 競合を防ぐために定義されています。 |
| `transactionManager` | `JdbcTransactionManager` | デフォルトトランザクションマネージャー (`@Primary`)。同じ型の ScalarDB 固有の bean との bean 競合を防ぐために定義されています。 |
| `scalarDbJdbcTemplate` | `ScalarDbJdbcTemplate` | カスタム例外変換を備えた ScalarDB 対応 JDBC テンプレート。 |
| `scalarDbNamedParameterJdbcTemplate` | `ScalarDbNamedParameterJdbcTemplate` | ScalarDB 対応名前付きパラメーターテンプレート。デフォルトで `@EnableScalarDbRepositories` によって使用されます。 |
| `scalarDbTransactionManager` | `ScalarDbTransactionManager` | ScalarDB トランザクションマネージャー。デフォルトで `@EnableScalarDbRepositories` によって使用されます。 |
| `scalarDbSuspendableTransactionManager` | `ScalarDbSuspendableTransactionManager` | 2フェーズコミットトランザクション用の ScalarDB トランザクションマネージャー。 |
...
Spring Data JDBC は1対多の関係をサポートしています。ただし、親の属性のみが変更された場合でも、関連付けられているすべての子レコードが暗黙的に削除され、再作成されます。この動作により、パフォーマンスが低下します。この懸念を考慮すると、Spring Data JDBC for ScalarDB でこの機能を使用することは推奨されません。

たとえば、Bank レコードに多数の Account レコードが含まれていると仮定すると、親エンティティに対して `BankRepository#update()` を呼び出すと、親の属性のみが変更された場合でも、Spring Data JDBC がすべての子 Account レコードを暗黙的に削除して再挿入します。
...
// `DELETE FROM "account" WHERE "account"."bank_id" = ?` が Spring Data JDBC によって暗黙的に発行され、
// その後すべての子レコードが再挿入されるため、不要なパフォーマンスオーバーヘッドが発生します。
...
ScalarDB SQL でネイティブにサポートされていない Java 型とデータベースカラム型間の変換が必要な場合 (例：`UUID`)、`AbstractJdbcConfiguration` を拡張してカスタムコンバーターを登録できます:
...
これは標準的な Spring Data JDBC パターンです。詳細については、Spring Data JDBC リファレンスの [カスタム変換](https://docs.spring.io/spring-data/jdbc/docs/3.0.x/reference/html/#jdbc.custom-converters)を参照してください。

### 2フェーズコミットトランザクション

:::warning 非推奨のお知らせ

ツーフェーズコミット (2PC) インターフェースは、ScalarDB 3.19 以降、非推奨となり、将来のリリースで削除される予定です。
...
### Spring Data JDBC との複数データソース設定

同じアプリケーションで標準の Spring Data JDBC リポジトリと並行して ScalarDB リポジトリを使用でき、それぞれ異なるデータソースによってバックアップできます。これは、アプリケーションがいくつかのテーブルを ScalarDB 経由で管理し、他のテーブルを標準の JDBC 接続を通じて管理する場合に有用です。

#### プロパティ

```console
# 非 ScalarDB Spring Data JDBC データソース
...
2つの `@Configuration` クラスを作成します。ScalarDB 側は `@EnableScalarDbRepositories` を使用し、非 ScalarDB 側は Spring Data JDBC の `@EnableJdbcRepositories` を使用します。`basePackages` を使用して、各アノテーションを正しいリポジトリインターフェースセットに向けます。
...
**非 ScalarDB Spring Data JDBC 設定:**
...
非 ScalarDB 側では標準の `JdbcTemplate` と `JdbcTransactionManager` を使用し、ScalarDB 側では `ScalarDbJdbcTemplate` と `ScalarDbTransactionManager` を使用します。

:::

#### `@Qualifier` を使用した bean 注入

アプリケーションコードで特定の `JdbcTemplate` bean を注入する必要がある場合は、`@Qualifier` を使用して正しいものを選択します:
...
Spring Data JDBC for ScalarDB は Spring Boot 2、3、および 4 でテストされています。
