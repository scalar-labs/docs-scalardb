...
### scalar.db.sql.default_namespace_name.existence_check.enabled

`scalar.db.sql.default_namespace_name.existence_check.enabled` が `true` (デフォルト) に設定されている場合、ScalarDB JDBC ドライバーは新しいセッションが作成されるたびにデフォルトの名前空間が存在することを確認します。Spring Data JDBC はアプリケーションの起動時に JDBC 接続を開くため、このチェックにより起動失敗が発生する可能性があります。

これを回避するには、JDBC URL で `scalar.db.sql.default_namespace_name.existence_check.enabled` を `false` に設定します。
