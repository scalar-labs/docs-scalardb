...
### `GroupBy` メソッドを使用してデータをグループ化および集計する
...
```c#
// HAVING 句を使用 (GroupBy の後の Where はグループをフィルタリングする)
_myDbContext.Statements
            .GroupBy(stat => stat.ItemId)
            .Where(g => g.Count() > 5)
            .Select(g => new { ItemId = g.Key, Count = g.Count() });
...
### `GroupBy` を使用せずにスタンドアロンの集計メソッドを使用する
...
以下のメソッドは SQL に変換され、クラスター上でリモート実行されます。チェーン可能なメソッドの場合、サポートされていないメソッドがこれらの後に続くと、LINQ to Objects を使用してローカルのメモリ内で実行されます。`First`/`FirstOrDefault` やスタンドアロン集計メソッドなどのターミナルメソッドは、結果を直接返します。
...
`GroupBy` の後の `Select` 内では、以下の集計関数がサポートされています:
...
- `GroupBy` の後では、以下のメソッドは SQL に変換されず、LINQ to Objects を使用してメモリ内でローカルに実行されます: `OrderBy`/`OrderByDescending`、`ThenBy`/`ThenByDescending`、`Skip`、`Take`、`First`/`FirstOrDefault`、`Join`、`GroupJoin`。
- ネストされた `GroupBy` (別の `GroupBy` の後に `GroupBy` を呼び出すこと) は SQL に変換されません。2 つ目の `GroupBy` は、LINQ to Objects を使用してメモリ内でローカルに実行されます。
- 述語付きの `Count` (例: `g.Count(x => x.Quantity > 5)`) は、`GroupBy` の後の `Select` や `Where` (HAVING) 内ではサポートされていません。グループ化の前に行をフィルタリングするには、代わりに `GroupBy` の前に `Where` を使用してください。
...
### 新しい読み取り専用トランザクションを開始する

トランザクション内で読み取り操作 (`AddAsync`、`UpdateAsync`、または `RemoveAsync` を伴わない LINQ クエリ) のみを実行する場合、次のように読み取り専用トランザクションを開始できます。
...
読み取り専用トランザクションは、書き込み操作が発行されないことを前提として、読み取り専用モードなしで開始された読み取りトランザクションよりも効率的に処理されます。そのため、読み取り専用トランザクション内で書き込み操作 (`AddAsync`、`UpdateAsync`、または `RemoveAsync`) を実行しようとするとエラーになります。読み取り専用モードは、2フェーズコミットインターフェースを使用したトランザクションではサポートされていません。
