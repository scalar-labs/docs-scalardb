...
`--coordinator` が指定されている場合は、[Coordinator テーブル](api-guide.mdx#coordinator-テーブルの操作を指定する)が作成されます。[グループコミット](consensus-commit.mdx#グループコミット)または [Coordinator の書き込みセットのロギング](consensus-commit.mdx#coordinator-の書き込みセットのロギング)が有効になっている場合、このコマンドは、有効になっている各機能が必要とする追加の列を持つ Coordinator テーブルを作成します。
...
Coordinator テーブルを作成した後に[グループコミット](consensus-commit.mdx#グループコミット)または [Coordinator の書き込みセットのロギング](consensus-commit.mdx#coordinator-の書き込みセットのロギング)を有効にした場合、`--coordinator` を指定してこのコマンドを実行すると、有効になっている各機能が必要とする追加の列も追加されます。
