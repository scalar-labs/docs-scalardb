...
| TIMESTAMP   | <em>サポートされていません</em>  | number (JSON)       | TIMESTAMP            | N        | datetime      | timestamp                | timestamp                | datetime2       | bigint  | number (JSON)  |
...
| ScalarDB | MySQL/MariaDB/TiDB  | PostgreSQL/YugabyteDB/AlloyDB | Oracle        | Db2            |
|----------|----------------|-----------------------|---------------|----------------|
...
| TIMESTAMPTZ  | 1000-01-01 00:00:00.000Z | 9999-12-31 23:59:59.999Z | 1 ミリ秒        |

このデータ型マッピングがアプリケーションと一致しない場合は、このツールを使用してテーブルを作成した後、テーブルを変更してデータ型を変更してください。

### データ型の制限事項

Db2 BLOB 型は、パーティションキー、クラスタリングキー、セカンダリインデックスキー、またはクロスパーティションスキャン操作の順序付け列として使用できません。

Oracle BLOB 型は、パーティションキー、クラスタリングキー、セカンダリインデックスキー、Get または Scan 操作の WHERE 条件の列、またはクロスパーティションスキャン操作の順序付け列として使用できません。

YugabyteDB には、浮動小数点型 (FLOAT および DOUBLE) が主キー、クラスタリングキー、またはセカンダリインデックスキーとして正しく機能しないという制限があります。

Object Storage では、1.5 GiB を超える BLOB 型データを格納することはできません。

## Consensus Commit の内部トランザクションメタデータ

Consensus Commit トランザクションマネージャーは、トランザクションを適切に処理するために、実際のレコードとともに保存されるトランザクションメタデータ (トランザクション ID、レコードバージョン、トランザクションステータスなど) を管理します。

したがって、アプリケーションに必要な列とともに、メタデータ用の追加列をスキーマに定義する必要があります。さらに、Consensus Commit トランザクションマネージャーを使用する場合、このツールはメタデータを含むテーブルを作成します。

### トランザクションメタデータ分離

[トランザクションメタデータ分離](./consensus-commit.mdx#トランザクションメタデータ分離)を有効にすることで、アプリケーションデータとは別にトランザクションメタデータを管理できます。

#### トランザクションメタデータ分離で新しいテーブルを作成する

トランザクションメタデータ分離で新しいテーブルを作成する場合、まずスキーマファイルに `"transaction-metadata-decoupling": true` を追加してオプションを有効にし、その後テーブルを作成する必要があります。
...
ScalarDB は、スキーマで定義されたテーブルごとに内部的に2つのテーブルを作成します。データテーブルとトランザクションメタデータテーブルは次のとおりです。

| テーブル                      | 説明                                                                                                              |
|----------------------------|--------------------------------------------------------------------------------------------------------------------------|
| `<table_name>_data`        | ユーザーデータを格納するアプリケーションテーブル                                                                                  |
| `<table_name>_tx_metadata` | トランザクション関連のメタデータと非プライマリキー列のビフォアイメージ列を格納するトランザクションメタデータテーブル |

その後、`INNER JOIN` を使用してデータテーブルとトランザクションメタデータテーブルを結合した仮想テーブルとして、テーブル `<table_name>` にアクセスできます。

#### トランザクションメタデータ分離で既存テーブルをインポートする

トランザクションメタデータ分離で既存のテーブルをインポートする場合も、スキーマファイルに `"transaction-metadata-decoupling": true` を追加してオプションを有効にし、その後テーブルをインポートする必要があります。
...
ScalarDB は、既存のテーブルをデータテーブルと想定し、次のようにトランザクションメタデータテーブルのみを作成します。

| テーブル                      | 説明                                                                                                              |
|----------------------------|--------------------------------------------------------------------------------------------------------------------------|
| `<table_name>`             | 元のアプリケーションテーブル（変更されません）                                                                                   |
| `<table_name>_tx_metadata` | トランザクション関連のメタデータと非プライマリキー列のビフォアイメージ列を格納するトランザクションメタデータテーブル |

その後、`LEFT OUTER JOIN` を使用してデータテーブルとトランザクションメタデータテーブルを結合した仮想テーブルとして、テーブル `<table_name>_scalardb` にアクセスできます。

#### 制限事項

現在、トランザクションメタデータ分離を使用する場合、次の操作はサポートされていません。
