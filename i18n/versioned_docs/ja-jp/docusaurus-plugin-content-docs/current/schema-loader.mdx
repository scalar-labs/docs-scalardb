...
| TIMESTAMP   | <em>サポートされていません</em> | number (JSON)       | N        | datetime      | timestamp                | timestamp                | datetime2       | bigint  |
...
ただし、JDBC データベースの次のデータ型は、主キーまたはセカンダリインデックスキーとして使用される場合、異なる方法で変換されます。これは、RDB データ型の制限によるものです。
