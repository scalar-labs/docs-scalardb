...
<Tabs groupId="databases" queryString>
  <TabItem value="MySQL" label="MySQL、MariaDB、と TiDB" default>
...
| nclob                          | TEXT                              |                                     |
| number(p,s), p ≠ 1の場合       | BIGINT / DOUBLE                   | 下記の警告 [5](#5) を参照してください。  |
| number(1,0)                    | BIGINT (デフォルト), BOOLEAN      | 下記の警告 [6](#6) を参照してください。  |
...
<ol>
  ...
  <li>
    <a name="7"></a>ScalarDB は、ScalarDB の `BLOB` 列としてインポートされた SQL Server の `image` 列のデータ型を `TEXT` に変更することをサポートしていません。
  </li>
</ol>
