...
詳細なデプロイ手順については、[ScalarDB Analytics サーバーをデプロイする](./deploy-scalardb-analytics-server.mdx)を参照してください。
...
- `<SCALARDB_ANALYTICS_VERSION>`: ScalarDB Analytics のバージョン (例: `3.18.0`)
...
<h3>ノートブックを使用したインタラクティブクエリの実行</h3>
...
EMR Studio ノートブックの設定および使用方法の詳細については、[Amazon EMR Studio ドキュメント](https://docs.aws.amazon.com/emr/latest/ManagementGuide/emr-studio.html)を参照してください。
...
Databricks Jobs の使用方法の詳細については、[Databricks ジョブのドキュメント](https://docs.databricks.com/ja/jobs/index.html)を参照してください。
...
Databricks ノートブックの使用方法の詳細については、[Databricks ノートブックのドキュメント](https://docs.databricks.com/ja/notebooks/index.html)を参照してください。
...
Databricks で JDBC を使用する方法の詳細については、[Databricks JDBC ドライバードキュメント](https://docs.databricks.com/ja/integrations/jdbc/index.html)を参照してください。
...
<h4>ノートブックを使用したインタラクティブクエリの実行</h4>

Azure Synapse Analytics ノートブックを使用して、ScalarDB Analytics を通じてインタラクティブクエリを実行することもできます。Spark プールを設定した後、ノートブックを作成して Spark プールにアタッチできます。

Azure Synapse Analytics ノートブックの使用方法の詳細については、[Azure Synapse Analytics ノートブックのドキュメント](https://learn.microsoft.com/ja-jp/azure/synapse-analytics/spark/apache-spark-notebook-concept)を参照してください。
...
- `<YOUR_LICENSE_CERT_PEM>`: ライセンス検証用の証明書。商用ライセンスと試用ライセンスの両方を含むライセンス設定方法については、[ライセンスキーの設定方法](../scalar-licensing/index.mdx)を参照してください。
...
Dataproc での Jupyter ノートブックの使用方法の詳細については、[Dataproc Jupyter コンポーネントのドキュメント](https://cloud.google.com/dataproc/docs/concepts/components/jupyter?hl=ja)を参照してください。
