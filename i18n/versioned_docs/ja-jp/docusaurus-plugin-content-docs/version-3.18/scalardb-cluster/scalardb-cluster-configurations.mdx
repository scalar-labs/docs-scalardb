...
- **説明:** クライアントがキャッシュする認証トークンの最大数。クライアントはユーザー名とパスワードのペアごとにトークンをキャッシュするため、リクエストのたびに認証する必要がありません。キャッシュされた各トークンは、キャッシュされてから1日後に有効期限が切れます。キャッシュは、設定プロパティで設定されたクレデンシャルとプログラムで渡されたクレデンシャルの間で共有されます。この設定は認証タイプが `userpass` の場合のみ適用されます。
- **デフォルト値:** `10000`
