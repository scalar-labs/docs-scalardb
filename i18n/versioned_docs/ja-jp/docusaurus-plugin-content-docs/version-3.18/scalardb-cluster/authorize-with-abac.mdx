...
#### トランザクションレベルで読み取りタグと書き込みタグを設定する

各オペレーションに読み取りタグや書き込みタグを指定する代わりに、`BEGIN` または `START TRANSACTION` の `WITH` 句を使用して、トランザクションの開始時に一度だけ指定できます。指定したタグはトランザクション内で発行されるすべてのオペレーションにマージされます。オペレーションに同じポリシーのタグがすでに設定されている場合は、オペレーション側のタグが優先されます。

トランザクションレベルでタグを設定する構文は次のとおりです。

```sql
BEGIN WITH ABAC_READ_TAG '<READ_TAG>' FOR POLICY <POLICY_NAME> AND ABAC_WRITE_TAG '<WRITE_TAG>' FOR POLICY <POLICY_NAME>;
```

構文の詳細については、ScalarDB SQL 文法の [BEGIN](../scalardb-sql/grammar.mdx#begin) セクションを参照してください。
...
次の設定ファイルを `scalardb-cluster-node.properties` として作成し、`<YOUR_LICENSE_KEY>` および `<LICENSE_CHECK_CERT_PEM>` を ScalarDB ライセンスキーおよびライセンスチェック証明書の値に置き換えます。ライセンスキーと証明書の詳細については、[製品ライセンスキーの設定方法](../scalar-licensing/index.mdx)を参照してください。
...
# Enable cross-partition scan (with filtering) to perform a full scan by using the SELECT statements in this tutorial.
# This is not required for ABAC itself.
...
services:
  ...
  scalardb-cluster-standalone:
    ...
    image: "ghcr.io/scalar-labs/scalardb-cluster-node-with-abac-byol-premium:3.18.1"
    ports:
      - 60053:60053
      - 9080:9080
    volumes:
...
ABAC 機能は現在、Private Preview です。詳細については、[お問い合わせ](https://www.scalar-labs.com/contact)いただくか、この機能が将来のバージョンで一般公開されるまでお待ちください。
