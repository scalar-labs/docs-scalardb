...
ScalarDB Java API は、主に Administrative API と Transaction API で設定されています。このガイドでは、どのような種類の API が存在するのか、それらの使用方法、例外の処理方法などの関連トピックについて簡単に説明します。
...
- Java API を使用した[テーブルの修復](#テーブルを修復する)
