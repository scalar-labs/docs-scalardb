...
| `AuthType`                  | `AuthEnabled` が `true` の場合に使用される認証タイプ。ユーザー名とパスワードによる認証の場合は `Userpass` を、OpenID Connect (OIDC) プロバイダーが発行した JWT アクセストークンを使用する認証の場合は `OidcJwt` を設定します。                                                                                                                                                                                                    | `Userpass`                                  |
| `AuthOidcJwtAccessToken`    | 認証に使用される OIDC JWT アクセストークン。`AuthEnabled` が `true` で `AuthType` が `OidcJwt` の場合に必須です。SDK はこのトークンを更新しないため、トークンの有効期限が切れたり無効になったりすると、認証エラーがアプリケーションに伝播されます。                                                                                                                                                                                                |                                             |
| `AuthTokenExpirationTime`   | 認証トークンを更新するまでの時間。このオプションは、ユーザー名とパスワードによる認証など、クラスターが発行する認証トークンを使用する認証方式に適用されます。`AuthOidcJwtAccessToken` で設定する OIDC JWT アクセストークンは更新しません。`AuthTokenExpirationTime` に設定された時間がクラスターの有効期限よりも長い場合、認証エラーを受信すると認証トークンが更新されます。認証トークンが正常に更新されると、認証エラーはクライアントコードに伝播されません。代わりに、認証エラーで失敗した操作が自動的に再試行されます。複数の操作が並行して実行されている場合、認証トークンが更新される前に、これらすべての操作が認証エラーで一度失敗します。 | `00:00:00` (クラスターから受信した認証トークンの有効期限が使用されます。) |
...
ScalarDB の値の範囲と精度の詳細については、[データベースアダプター](../database-adapters.mdx#値範囲と精度)を参照してください。
