...
- FLOAT カラムは、パーティションキー、クラスタリングキー、またはセカンダリインデックスキーとして使用できません。
- テーブルとカラムの名前変更はサポートされていません。
- ALTER COLUMN TYPE は BLOB → TEXT 変換のみをサポートしています。
- LIKE 式のエスケープ文字は `\` に固定されており、設定や無効化はできません。
...
- BIGINT 値は -2^53 から 2^53 の範囲に制限されています。Cosmos DB は数値を内部的に倍精度浮動小数点数として保存するため、2^53 までの整数のみを正確に表すことができます。
...
- BIGINT 値は -2^53 から 2^53 の範囲に制限されています。これは、数値が JSON 数値としてシリアライズされ、2^53 までの整数のみを正確に表すことができます。
...
ScalarDB は、基盤となるデータベースのネイティブ機能に関係なく、ほとんどの場合、すべてのアダプターで一貫した値範囲を強制します。以下の表は、各データ型でサポートされる範囲と精度を示しています。

| データ型 | 範囲 | 精度 | 備考 |
|-----------|-------|-----------|-------|
| BOOLEAN | `true` または `false` | | |
| INT | -2^31 to 2^31 - 1 | | |
| BIGINT | -2^63 to 2^63 - 1 | | Cosmos DB for NoSQL およびオブジェクトストレージアダプターでは -2^53 から 2^53 に制限されています。 |
