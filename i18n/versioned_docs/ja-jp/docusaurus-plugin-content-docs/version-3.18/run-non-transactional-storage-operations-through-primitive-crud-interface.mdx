...
既存のストレージおよびデータベースシステム上でストレージに依存しない、またはデータベースに依存しない ACID トランザクションを実現するためのキーの1つは、ScalarDB が提供するストレージ抽象化機能です。ストレージ抽象化は、[データモデル](./data-modeling.mdx)と、データモデルに基づいて操作を発行する API (Storage API) を定義します。

ほとんどの場合、[Transaction API](api-guide.mdx#transaction-api) を使用することになりますが、別のオプションとして Storage API を使用することもできます。

Storage API を使用する利点は次のとおりです。

- Transaction API と同様に、基盤となるストレージ実装についてあまり気にせずにアプリケーションコードを作成できます。
- アプリケーション内の一部のデータに対してトランザクションが必要ない場合は、Storage API を使用してトランザクションを部分的に回避し、実行を高速化できます。

:::warning

Storage API を直接使用したり、Transaction API と Storage API を混在させたりした場合、予期しない動作が発生する可能性があります。たとえば、Storage API はトランザクション機能を提供できないため、操作の実行時に障害が発生すると、API によって異常やデータの不整合が発生する可能性があります。

したがって、Storage API の使用には *非常に* 注意し、何をしているのかを正確に理解している場合にのみ使用してください。

:::

## Storage API の例

このセクションでは、基本的な電子マネーアプリケーションで Storage API を使用する方法について説明します。
...
これを念頭に置いて、この Storage API の例では、設定ファイル `scalardb.properties` が存在することを前提としています。
...
Storage API は、Administrative API と CRUD API で構成されています。
...
作成オプションの詳細については、[作成オプション](./api-guide.mdx#作成オプション-2)を参照してください。
