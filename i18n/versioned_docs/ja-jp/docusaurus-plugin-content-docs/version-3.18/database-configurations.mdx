...
<Tabs groupId="databases" queryString>
  ...
  <TabItem value="NoSQL_databases" label="NoSQL データベース">
    NoSQL データベースを選択してください。
    <Tabs groupId="nosql-databases" queryString>
  ...
  <TabItem value="Object_Storage" label="Object Storage">
    ...
    - すべての操作に単一のリージョンを使用します。Blob Storage の場合は、単一のプライマリリージョンを使用してください。
    - 各ストレージに対して次のストレージクラスまたはアクセス層を選択します：
      - **S3:** [S3 標準](https://aws.amazon.com/jp/s3/storage-classes/)
      - **Blob Storage:** [Standard 汎用 v2、ホットアクセス層](https://learn.microsoft.com/ja-jp/azure/storage/blobs/access-tiers-overview)
      - **Cloud Storage:** [Standard ストレージ](https://docs.cloud.google.com/storage/docs/storage-classes?hl=ja)

:::note

上記以外のストレージクラスまたはアクセス層も使用可能ですが、上記のものは検証済みでサポートされています。
...
:::
  ...
  <TabItem value="Object_Storage" label="Object Storage">
    - S3 を使用する場合は、[不完全なマルチパートアップロードを削除するためのバケットライフサイクル設定の構成](https://docs.aws.amazon.com/ja_jp/AmazonS3/latest/userguide/mpu-abort-incomplete-mpu-lifecycle-config.html)で説明されているように、不完全なマルチパートアップロードを削除するライフサイクルルールを設定してください。
    - オブジェクトストレージ固有のスキーマ設計の推奨事項については、[データベースアダプター](database-adapters.mdx#キーとインデックスマッピング-4)を参照してください。
  </TabItem>
</Tabs>
