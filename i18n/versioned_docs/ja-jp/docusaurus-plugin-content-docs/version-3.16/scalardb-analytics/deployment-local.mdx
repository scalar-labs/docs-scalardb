...
- `<YOUR_LICENSE_CERT_PEM>`: ScalarDB Analytics サーバーの PEM エンコードされたライセンス証明書。商用ライセンスと試用ライセンスの両方を含むライセンス設定方法については、[ライセンスキーの設定方法](../scalar-licensing/index.mdx)を参照してください。
...
前述したように、ScalarDB Analytics サーバーと Spark ドライバーの両方が、同じパスで ScalarDB データソース設定ファイル (`scalardb.properties`) にアクセスする必要があります。そのため、この例では、`ConfigMap` に保存されたファイルを `/tmp/scalar` パスにマウントしています。
