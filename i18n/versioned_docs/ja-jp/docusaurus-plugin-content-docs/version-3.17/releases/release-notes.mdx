...
このリリースでは、Cosmos DB のエラー処理を改善し、スキーマ操作中の競合エラーに対するリトライ処理を追加し、ScalarDB SQL における二相コミットトランザクションのスレッドプール管理を改善し、セキュリティの脆弱性に対処するために AWS SDK と grpc_health_probe をアップグレードしました。
