...
- 暗号化された列は、基になるデータベースに BLOB 型として保存されるため、BLOB 型の最大サイズを超える暗号化された列は保存できません。使用しているデータベースで BLOB 型の最大サイズが定められているかどうかについては、[データベースアダプター](../database-adapters.mdx)を参照してください。
