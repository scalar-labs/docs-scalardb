...
<Tabs groupId="databases" queryString>
  ...
  <TabItem value="Object_Storage" label="Object Storage">
    ...
    - 各ストレージに対して次のストレージクラスまたはアクセス層を選択します：
      ...
      - **Blob Storage:** [ホットアクセス層](https://learn.microsoft.com/ja-jp/azure/storage/blobs/access-tiers-overview)
...
:::
  ...
  <TabItem value="Object_Storage" label="Object Storage">
    - ScalarDB のパーティションは Object Storage のオブジェクトにマップされるため、次のようにテーブルスキーマを設計することが推奨されます：
      - レコードのサイズが大きい場合は、クラスタリングキーの定義を避けてください。
      - Object Storage への読み取り操作の数を減らすために、頻繁に読み取られるレコードを同じパーティション内に配置してください。
      - Object Storage での書き込み競合を避けるために、頻繁に書き込まれるレコードは異なるパーティションに配置してください。
