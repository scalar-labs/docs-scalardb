---
tags:
  - Enterprise Standard
  - Enterprise Premium
displayed_sidebar: docsEnglish
---

# How to Configure a Trial License Key

You can use the following trial license keys for ScalarDB Enterprise Premium and ScalarDB Analytics. If you have a commercial license key, please refer to [How to Configure a Product License Key](./index.mdx) to configure your license key.

:::warning

- This key is valid for evaluation purposes only.
- When using a trial license, ScalarDB Cluster and/or ScalarDB Analytics must be connected to the Internet to validate the license and check its expiration.
- The trial license key is updated periodically. For production use, please [contact us](https://www.scalar-labs.com/contact) to obtain a commercial license.

:::

:::note

ScalarDB Core is available as an open-source software under the Apache 2.0 License on [GitHub](https://github.com/scalar-labs/scalardb).

:::

## ScalarDB Enterprise Premium

```properties
scalar.db.cluster.node.licensing.license_key={"organization_name":"Trial","product_name":"ScalarDB Cluster","product_version":3,"license_type":"trial","signature":"MEYCIQDoYXyiOxtQhyT0+PbsrYZFjP6l1AO/FWmgC+tEi/rQNAIhALhyP40VRzOEBF431pDTFrqazR2puxyVSPEP2XRWpG80","expiration_date_time":"2026-05-18T10:30:09.324+09:00[Asia/Tokyo]"}
scalar.db.cluster.node.licensing.license_check_cert_pem=-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----
```

## ScalarDB Analytics

```apacheconf
spark.sql.catalog.scalardb_catalog.license.key {"organization_name":"Trial","product_name":"ScalarDB Analytics","product_version":3,"license_type":"trial","signature":"MEUCIF39FSkr0yz9IyNsuAzVuixq5EmPsJSUzkHWAmWONYaEAiEA+RCYxhGMzm9FJmOvRvrD+jnPDzWloGIxM4WU3hgjVwQ=","expiration_date_time":"2026-05-18T10:30:10.985+09:00[Asia/Tokyo]"}
spark.sql.catalog.scalardb_catalog.license.cert_pem -----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----
```
