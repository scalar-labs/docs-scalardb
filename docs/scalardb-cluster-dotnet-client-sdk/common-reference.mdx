...
When using [LINQ](getting-started-with-linq.mdx#set-up-classes) or extension methods for the [Transaction API](getting-started-with-scalardb-tables-as-csharp-classes.mdx#create-classes-for-all-scalardb-tables), [SQL API](getting-started-with-distributed-sql-transactions.mdx#execute-sql-queries), or [Administrative API](getting-started-with-scalardb-tables-as-csharp-classes.mdx#use-the-administrative-api), a column's value received from the cluster is automatically converted to a corresponding .NET type. Likewise, a value of a .NET property is automatically converted to a corresponding cluster's type when an object is being saved to the cluster.
...
For details about value ranges and precision in ScalarDB, see [Database Adapters](../database-adapters.mdx#value-ranges-and-precision).
