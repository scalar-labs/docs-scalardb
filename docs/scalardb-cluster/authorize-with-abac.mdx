...
- Since your write tag includes the `C` level, you can write data with the levels `C` and `P`. You cannot write data with the levels `S` and `HS` because they are higher than your authorized level. Therefore, you cannot write the rows `1`, and `3`.
