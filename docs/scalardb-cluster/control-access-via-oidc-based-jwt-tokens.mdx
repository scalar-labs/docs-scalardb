...
Both mechanisms work in the same way for the `userpass` and `oidc_jwt` authentication types and are described in [Passing credentials programmatically](./scalardb-auth-with-sql.mdx#passing-credentials-programmatically) in the authentication and authorization guide.
