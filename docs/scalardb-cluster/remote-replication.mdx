...
Multi-region databases may have higher latency. Consider using [group commit](../consensus-commit.mdx#group-commit) to improve throughput on high-latency Coordinator databases. The Coordinator group commit feature batches multiple transactions together, reducing the impact of network latency on overall throughput. For replication database tuning, see the [LogWriter performance tuning](#logwriter-performance-tuning) section above.
...
Replace `<PRIMARY_CLUSTER_CONTACT_POINTS>` with your primary site cluster contact points (same format as [ScalarDB Cluster Java Client SDK configurations](scalardb-cluster-configurations.mdx#java-client-sdk-configurations)) and `<VERSION>` with the ScalarDB Cluster version that you're using.
...
Replace `<BACKUP_CLUSTER_CONTACT_POINTS>` with your backup site cluster contact points (same format as [ScalarDB Cluster Java Client SDK configurations](scalardb-cluster-configurations.mdx#java-client-sdk-configurations)) and `<VERSION>` with the ScalarDB Cluster version that you're using.
...
Replace `<BACKUP_CLUSTER_CONTACT_POINTS>` with your backup site cluster contact points (in the same format as [ScalarDB Cluster Java Client SDK configurations](scalardb-cluster-configurations.mdx#java-client-sdk-configurations)) and `<VERSION>` with the ScalarDB Cluster version that you're using.
