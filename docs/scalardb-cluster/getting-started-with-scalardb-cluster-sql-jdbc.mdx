...
To load a schema, you need to use [the SQL CLI](developer-guide-for-scalardb-cluster-with-java-api.mdx#scalardb-cluster-sql-cli). You can download the SQL CLI from [ScalarDB Releases](https://github.com/scalar-labs/scalardb/releases/tag/v3.19.0). After downloading the JAR file, you can use SQL CLI for Cluster by running the following command:
