...
<Tabs groupId="databases" queryString>
  ...
  <TabItem value="Spanner" label="Spanner">
    ...
    You can run Spanner Omni in Docker Compose by using the `docker-compose.yaml` file in the `scalardb-samples/scalardb-cluster-standalone-mode` directory.
    ...
    The **scalardb-cluster-node.properties** file in the `scalardb-samples/scalardb-cluster-standalone-mode` directory contains database configurations for ScalarDB Cluster. Please uncomment the properties for Spanner in the **scalardb-cluster-node.properties** file so that the configuration looks as follows:
