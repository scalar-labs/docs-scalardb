...
- **OIDC (`OIDC`):** Client applications pass JWT access tokens from an OIDC provider (for example, Keycloak) instead of passwords. For details, see [Integrate OIDC for Access Control](./integrate-oidc-for-access-control.mdx).
