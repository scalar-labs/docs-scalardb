...
| DATE        | date                 | number (JSON)       | N        | date          | date                     | date                     | date            | int     |
| TIME        | time                 | number (JSON)       | N        | time          | time                     | timestamp                | time            | bigint  |
| TIMESTAMP   | <em>Unsupported</em> | number (JSON)       | N        | datetime      | timestamp                | timestamp                | datetime2       | bigint  |
| TIMESTAMPTZ | timestamp            | number (JSON)       | N        | datetime      | timestamp with time zone | timestamp with time zone | datetimeoffset  | bigint  |
