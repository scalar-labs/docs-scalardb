...
- Whether the [group commit feature for the Coordinator table](api-guide.mdx#group-commit-for-coordinator-table) is enabled or not, if the `--coordinator` option described below is specified
...
TIMESTAMP represents a date-time without time zone information, while TIMESTAMPTZ represents a date-time on the UTC time zone.  
...
The following data types have a value range and precision regardless of the underlying database.

| Type        | Minimum                   | Maximum                   | Precision     |
|-------------|---------------------------|---------------------------|---------------|
| BIGINT      | -2^53                     | 2^53                      | -             |
| DATE        | 1000-01-01                | 9999-12-31                | 1 day         |
| TIME        | 00:00:00.000000           | 23:59:59.999999           | 1 microsecond |
| TIMESTAMP   | 1000-01-01 00:00:00.000   | 9999-12-31 23:59:59.999   | 1 millisecond |
| TIMESTAMPTZ | 1000-01-01 00:00:00.000 Z | 9999-12-31 23:59:59.999 Z | 1 millisecond |
...
Db2 BLOB type cannot be used as a partition key, clustering key, secondary index key, or as an ordering column in a cross-partition Scan operation.

Oracle BLOB type cannot be used as a partition key, clustering key, secondary index key, a column of a where condition in a Get or Scan operation, or as an ordering column in a cross-partition Scan operation.
...
You can separately manage the transaction metadata from application data by enabling [transaction metadata decoupling](./consensus-commit.mdx#transaction-metadata-decoupling).
