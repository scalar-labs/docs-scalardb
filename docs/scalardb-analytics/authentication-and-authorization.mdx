...
ScalarDB Analytics can authenticate and authorize users with fine-grained, resource-level access control. You can create users and grant or revoke permissions on catalogs, data sources, namespaces, and tables. Roles can also be created to group permissions and can be granted to users. This guide describes how to use authentication and authorization in ScalarDB Analytics.
...
Users can log in to ScalarDB Analytics with a username and a password and execute operations if they have the required permissions.

Authentication and authorization support two types of users:

- **Superusers:** This type of user has all permissions. Only superusers can create or delete catalogs and manage users and roles.
...
When the server starts, ScalarDB Analytics creates the user specified by `scalar.db.analytics.server.auth.initial-admin-username` with the password specified by `scalar.db.analytics.server.auth.password.internal.initial-admin-password` and assigns the SUPERADMIN role to that user. This assignment occurs only if no users are already assigned to the SUPERADMIN role. You can log in with this user and create other users if necessary.

#### ScalarDB Cluster backend

The server does not create any users at startup. Instead, when the user specified by `scalar.db.analytics.server.auth.initial-admin-username` logs in for the first time, ScalarDB Analytics automatically registers that user through just-in-time provisioning and assigns the SUPERADMIN role. This assignment occurs only if no users are already assigned to the SUPERADMIN role. Other users are also provisioned on first login, but they are not assigned the SUPERADMIN role.
...
When [ScalarDB authorization delegation](#scalardb-authorization-delegation) is enabled, namespace-level and table-level authorization for ScalarDB data sources is delegated to ScalarDB Cluster instead of following the permissions listed in the table below.
...
When you use ScalarDB Cluster as the authentication backend, you can optionally delegate namespace-level and table-level authorization for ScalarDB data sources to ScalarDB Cluster. This means that ScalarDB Cluster's privilege system becomes the single source of truth for access control on ScalarDB data, eliminating the need to manage permissions in two places.

### How delegation works

When authorization delegation is enabled, ScalarDB Analytics checks ScalarDB Cluster privileges (specifically, the `SELECT` privilege) instead of its own access control entries for namespace and table operations on ScalarDB data sources. Catalog-level and data source-level authorization remains under ScalarDB Analytics access control regardless of delegation settings.
...
- The `scalar.db.analytics.server.auth.password.scalardb-cluster.acl-delegation` property must be set to `true`.
...
| Namespace (ScalarDB)          | ScalarDB Cluster   | ScalarDB Analytics  |
| Table (ScalarDB)              | ScalarDB Cluster   | ScalarDB Analytics  |
| Namespace (non-ScalarDB)      | ScalarDB Analytics | ScalarDB Analytics  |
| Table (non-ScalarDB)          | ScalarDB Analytics | ScalarDB Analytics  |
...
|-------------------------------------------------|------------------------------------------------------|------------|
| `scalar.db.analytics.server.auth.enabled`       | Whether authentication and authorization are enabled. | `false`    |

You can also set the following configurations:

| Name                                                                       | Description                                                                                           | Default    |
|----------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------|------------|
| `scalar.db.analytics.server.auth.initial-admin-username`                   | The username of the initial admin user who will be assigned the SUPERADMIN role.                       |            |
| `scalar.db.analytics.server.auth.password.backend`                         | The authentication backend to use (`internal` or `scalardb-cluster`).                                 | `internal` |
| `scalar.db.analytics.server.auth.password.token-ttl-seconds`               | The time-to-live (TTL) of access tokens in seconds.                                                   | `86400`    |
| `scalar.db.analytics.server.auth.password.internal.initial-admin-password` | The initial password for the admin user. Only used when `auth.password.backend` is set to `internal`.  |            |

If you use `scalardb-cluster` as the authentication backend, you can also set the following configurations:

| Name                                                                               | Description                                                                                                          | Default     |
|------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------|-------------|
| `scalar.db.analytics.server.auth.password.scalardb-cluster.host`                   | The hostname or IP address of the ScalarDB Cluster instance.                                                         | `localhost` |
| `scalar.db.analytics.server.auth.password.scalardb-cluster.port`                   | The port number of the ScalarDB Cluster instance.                                                                    | `60053`     |
| `scalar.db.analytics.server.auth.password.scalardb-cluster.deadline-millis`         | The gRPC deadline in milliseconds for authentication requests to ScalarDB Cluster.                                   | `5000`      |
| `scalar.db.analytics.server.auth.password.scalardb-cluster.acl-delegation`          | Whether to delegate namespace-level and table-level authorization for ScalarDB data sources to ScalarDB Cluster. For details, see [ScalarDB authorization delegation](#scalardb-authorization-delegation). | `false`     |
| `scalar.db.analytics.server.auth.password.scalardb-cluster.tls.enabled`             | Whether to enable TLS for the connection to ScalarDB Cluster.                                                        | `false`     |
| `scalar.db.analytics.server.auth.password.scalardb-cluster.tls.ca-root-cert-path`   | Path to the CA root certificate file for verifying the ScalarDB Cluster server certificate.                          |             |
| `scalar.db.analytics.server.auth.password.scalardb-cluster.tls.override-authority`   | Override the server authority for TLS verification of the ScalarDB Cluster connection.                                |             |
...
| Name                                        | Description                                                          | Default |
|---------------------------------------------|----------------------------------------------------------------------|---------|
| `scalar.db.analytics.client.auth.username`  | The username for authenticating with the ScalarDB Analytics server.   |         |
| `scalar.db.analytics.client.auth.password`  | The password for authenticating with the ScalarDB Analytics server.   |         |
...
| Name                                                          | Description                                                                              | Default |
|---------------------------------------------------------------|------------------------------------------------------------------------------------------|---------|
| `spark.sql.catalog.<catalog-name>.server.auth.username`       | The username for authenticating the Spark application with the ScalarDB Analytics server. |         |
| `spark.sql.catalog.<catalog-name>.server.auth.password`       | The password for authenticating the Spark application with the ScalarDB Analytics server. |         |

## Wire encryption

If you enable authentication and authorization, you must also enable TLS to protect user credentials in transit. For details about TLS configuration, see [ScalarDB Analytics Configurations](configurations.mdx).

## Next steps

- [ScalarDB Analytics Configurations](configurations.mdx) - View all available configuration options
