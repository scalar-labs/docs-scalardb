...
When `scalar.db.sql.default_namespace_name.existence_check.enabled` is set to `true` (default), the ScalarDB JDBC driver verifies that the default namespace exists each time a new session is created. Because Spring Data JDBC opens a JDBC connection during application startup, this check can cause a startup failure.

To avoid this, set `scalar.db.sql.default_namespace_name.existence_check.enabled` to `false` in the JDBC URL.
