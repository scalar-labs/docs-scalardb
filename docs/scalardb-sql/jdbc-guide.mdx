...
Batch execution is restricted to mutation statements (`INSERT`, `UPSERT`, `UPDATE`, and `DELETE`). Passing a `SELECT`, DDL, or DCL statement to `addBatch(...)` causes the subsequent `executeBatch()` call to throw a `SQLException`. For batch execution that also accepts DDL and DCL statements, see [Execute batch statements](sql-api-guide.mdx#execute-batch-statements) in the ScalarDB SQL API Guide.
